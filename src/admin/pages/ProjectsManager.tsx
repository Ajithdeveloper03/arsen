        ...
        <div className="space-y-6 max-w-7xl mx-auto pb-20">
            ...
            <div className="bg-white border border-slate-200 p-4 rounded-xl flex flex-col md:flex-row gap-4 items-center justify-between shadow-sm sticky top-4 z-10">
                <div className="relative w-full md:w-96">
                    ...
                    <input type="text" placeholder="Search projects..." className="w-full bg-slate-50 border border-slate-200 rounded-lg py-2.5 pl-10 pr-4 text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#022C22]/10 focus:border-[#022C22] transition-all text-sm font-medium"
                        value={search} onChange={e => setSearch(e.target.value)} />
                </div>
                <div className="flex items-center gap-3 w-full md:w-auto overflow-x-auto">
                    <div className="bg-slate-100 p-1 rounded-lg flex border border-slate-200 shrink-0">
                        <button onClick={() => setViewMode('grid')} className={`p-2 rounded-md transition-all ${viewMode === 'grid' ? 'bg-white shadow-sm text-[#022C22]' : 'text-slate-400 hover:text-slate-600'}`} title="Grid View">
                            <LayoutGrid size={18} />
                        </button>
                        <button onClick={() => setViewMode('table')} className={`p-2 rounded-md transition-all ${viewMode === 'table' ? 'bg-white shadow-sm text-[#022C22]' : 'text-slate-400 hover:text-slate-600'}`} title="Table View">
                            <List size={18} />
                        </button>
                    </div>
                    <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)} className="px-4 py-2 bg-slate-100 rounded-lg text-sm font-bold text-slate-700 border-none focus:ring-2 focus:ring-[#022C22]/10">
                    ...
                    <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="px-4 py-2 bg-slate-100 rounded-lg text-sm font-bold text-slate-700 border-none focus:ring-2 focus:ring-[#022C22]/10">
            ...
            {loading ? (
                ...
                <div className="bg-white p-20 rounded-2xl shadow-sm border border-slate-200 text-center text-slate-400">
                    ...
                    <p className="font-bold uppercase tracking-widest text-sm">No projects found</p>
                    <p className="text-xs mt-2">Add your first project using the "Add Project" button above.</p>
                </div>
            ) : viewMode === 'grid' ? (
