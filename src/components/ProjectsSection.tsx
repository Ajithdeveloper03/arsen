...
import gallery3 from '../assets/projected1.jpeg';
...
import gallery9 from '../assets/projected2.jpg';
import gallery10 from '../assets/gallery-commercial6.jpg';
import gallery11 from '../assets/projected3.jpg';
      ...
      <div className="flex flex-col gap-4 md:gap-8">
        <div className="flex overflow-hidden group">
          <div className="flex gap-4 md:gap-6 animate-marquee whitespace-nowrap items-center py-4">
            {projectsRow1.map((img, i) => <ProjectCard key={i} image={img} index={i} />)}
            {projectsRow1.map((img, i) => <ProjectCard key={i + 'c'} image={img} index={i} />)}
          </div>
        </div>

        <div className="flex overflow-hidden group">
          <div className="flex gap-4 md:gap-6 animate-marquee-reverse whitespace-nowrap items-center py-4">
            {projectsRow2.map((img, i) => <ProjectCard key={i} image={img} index={i} />)}
            {projectsRow2.map((img, i) => <ProjectCard key={i + 'c'} image={img} index={i} />)}
          </div>
        </div>
      </div>
